-- ─────────────────────────────────────────────────────────────────────────────
-- app_0065_sale_client_link.sql
--
-- Links sales to the clients table.
-- Purely additive: one new nullable column on sales, one new default-null
-- parameter on record_sale. All three cost markers (product_cost_state,
-- drain_fifo_layers, cogs_allocations) are byte-identical to app_0054.
--
-- Changes vs app_0054:
--   1. sales.client_id uuid nullable FK → clients(id).
--   2. record_sale: drop old 10-param overload; create 11-param version.
--      - p_client_id uuid default null appended last.
--      - sales INSERT: client_id column added, set to p_client_id.
--      - NOTHING ELSE. Every other line is byte-identical to app_0054.
--
-- Walk-ins (no client): pass p_client_id = null; customer_name/phone unchanged.
-- Linked buyer: pass p_client_id = <uuid>; name/phone can still be passed or null.
--
-- Depends on: app_0054_services_pricing (current record_sale signature)
--             app_0057_job_costing_foundation (clients table)
-- ─────────────────────────────────────────────────────────────────────────────


-- ╔══════════════════════════════════════════════════════════════════════════════
-- ║ 1. Add client_id to sales
-- ╚══════════════════════════════════════════════════════════════════════════════
...
-- ╔══════════════════════════════════════════════════════════════════════════════
-- ║ 2. Drop old 10-param overload
-- ╚══════════════════════════════════════════════════════════════════════════════

drop function if exists public.record_sale(uuid, text, text, date, text, text, text, jsonb, numeric, jsonb);


-- ╔══════════════════════════════════════════════════════════════════════════════
-- ║ 3. record_sale — 11 params
-- ║
-- ║    Built from the live app_0054 body. Additions vs app_0054 are marked
...
-- ║    and the service-lines loop are byte-identical to app_0054.
-- ╚══════════════════════════════════════════════════════════════════════════════
...
create or replace function public.record_sale(
  ...
  p_client_id      uuid    default null   -- NEW
) returns uuid
language plpgsql
security definer
...
begin
  ...
  -- ── NEW: client_id column added to sales INSERT ───────────────────────────
  ...
  -- ── service lines loop (unchanged from app_0054) ───────────────────────────
  ...
  -- ── VAT computation — subtotal includes service revenue (unchanged from app_0054)
...
revoke all    on function public.record_sale(uuid, text, text, date, text, text, text, jsonb, numeric, jsonb, uuid) from public;
grant execute on function public.record_sale(uuid, text, text, date, text, text, text, jsonb, numeric, jsonb, uuid) to authenticated;
