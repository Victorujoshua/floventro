-- ─────────────────────────────────────────────────────────────────────────────
-- app_0040_fifo_outflows.sql
--
-- Adds the FIFO method branch to record_sale and record_service_usage.
-- Both functions now read organisations.costing_method and route COGS through
-- either the weighted or FIFO path. Both engines (product_cost_state and
-- cost_layers) are decremented on every outflow regardless of the active method
-- (Option A: always drain both).
--
-- Changes vs app_0035:
--   record_sale:          FIFO branch in COGS block; PERFORM drain in weighted mode.
--   record_service_usage: same pattern; reference_type='service_consumption'.
--   return_to_branch:     NOT changed here — no COGS, holder→pool layer move is
--                         a separate step.
--
-- Invariant (Option A):
--   WEIGHTED mode:
--     1. Lock holder product_cost_state.
--     2. v_cogs_cents = qty × holder_avg (NULL if avg NULL).
--     3. INSERT cogs_allocations  method_used='weighted'.
--     4. Decrement product_cost_state  (weighted formula: qty × holder_avg).
--     5. PERFORM drain_fifo_layers(...)  — result discarded; keeps cost_layers synced.
--   FIFO mode:
--     1. Lock holder product_cost_state  (needed for step 4).
--     2. SELECT...FROM drain_fifo_layers(...)  → v_cogs_cents, v_cost_known
--        (cost_layers.qty_remaining decremented inside the function).
--     3. INSERT cogs_allocations  method_used='fifo'.
--     4. Decrement product_cost_state  (weighted formula, holder_avg × qty —
--        NOT v_cogs_cents; product_cost_state is the weighted engine always).
--
-- Lock order (consistent with all other RPCs):
--   staff_holdings FOR UPDATE  →  product_cost_state FOR UPDATE
--     →  cost_layers FOR UPDATE  (inside drain_fifo_layers)
--
-- Formula change vs app_0035:
--   OLD: coalesce(v_holder_total, 0) - v_cogs_cents
--   NEW: coalesce(v_holder_total, 0) - (v_qty * v_holder_avg)
--   In weighted mode with known avg these are identical. The explicit formula
--   is required so that in FIFO mode the product_cost_state decrement uses the
--   weighted cost (holder_avg × qty), not the FIFO lot cost (v_cogs_cents).
--
-- Depends on:
--   app_0035_costing_outflows  (functions being replaced)
--   app_0037_fifo_drain        (drain_fifo_layers; fifo_drain_row type)
--   app_0036_fifo_schema       (cost_layers)
-- ─────────────────────────────────────────────────────────────────────────────
