...
export async function sendEmail(params: {
  ...
  // ── Diagnostic logging ───────────────────────────────────────────────────
  console.log("[zeptomail:diag] ZEPTOMAIL_TOKEN:", token
    ? `present | length=${token.length} | first4="${token.slice(0, 4)}"`
    : "MISSING")
  console.log("[zeptomail:diag] ZEPTOMAIL_FROM:", fromAddress ?? "MISSING")
  // ─────────────────────────────────────────────────────────────────────────
  ...
  try {
    const res = await fetch("https://api.zeptomail.com/v1/email", {
      method: "POST",
      headers: {
    ...
    console.log("[zeptomail] send ok | status:", res.status, "| body:", successBody)
    return { ok: true }
  } catch (err) {
